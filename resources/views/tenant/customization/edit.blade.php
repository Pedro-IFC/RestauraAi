                ...
                <div class="mt-5 grid gap-5 md:grid-cols-2">
                    <div>
                        ...
                        <div class="mt-1 flex gap-2">
                            <input id="primary_color" type="color" value="{{ old('primary_color', $customization->primary_color ?? '#2563eb') }}"
                                class="h-10 w-14 rounded-lg border border-gray-300" data-color-picker="primary_color" @disabled(! $canUseAdvancedCustomization)>
                            <input id="primary_color_text" name="primary_color" type="text" value="{{ old('primary_color', $customization->primary_color ?? '#2563eb') }}"
                                class="min-w-0 flex-1 rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" @readonly(! $canUseAdvancedCustomization)>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex gap-2">
                            <input id="secondary_color" type="color" value="{{ old('secondary_color', $customization->secondary_color ?? '#f3f4f6') }}"
                                class="h-10 w-14 rounded-lg border border-gray-300" data-color-picker="secondary_color" @disabled(! $canUseAdvancedCustomization)>
                            <input id="secondary_color_text" name="secondary_color" type="text" value="{{ old('secondary_color', $customization->secondary_color ?? '#f3f4f6') }}"
                                class="min-w-0 flex-1 rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" @readonly(! $canUseAdvancedCustomization)>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-5">
                    ...
                    @if ($customization->logo)
                        <label class="mt-3 flex items-center gap-2 text-sm text-gray-700">
                            <input type="checkbox" name="remove_logo" value="1" class="rounded border-gray-300 text-blue-600" @disabled(! $canUseAdvancedCustomization)>
                            Remover logo atual
                        </label>
                    @endif
                </div>
                        ...
                        @foreach ($customization->banners as $banner)
                            <label class="overflow-hidden rounded-lg border border-gray-200 bg-gray-50">
                                ...
                                <span class="flex items-center gap-2 p-3 text-sm text-gray-700">
                                    <input type="checkbox" name="remove_banners[]" value="{{ $banner }}" class="rounded border-gray-300 text-blue-600" @disabled(! $canUseAdvancedCustomization)>
                                    Remover este banner
                                </span>
                            </label>
                        @endforeach
                ...
                <div class="mt-5 space-y-5">
                    ...
                    <div>
                        ...
                        <input id="instagram_handle" name="instagram_handle" type="text" value="{{ old('instagram_handle', $customization->instagram_handle) }}"
                            ...
                            class="mt-1 w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" @readonly(! $canUseAdvancedCustomization)>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="address_text" name="address_text" rows="3"
                            class="mt-1 w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('address_text', $customization->address_text) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="google_maps_iframe" name="google_maps_iframe" rows="5"
                            class="mt-1 w-full rounded-lg border-gray-300 font-mono text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500"
        ...
        <aside class="space-y-6">
            <section class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
                ...
                <div class="mt-4 overflow-hidden rounded-lg border border-gray-200">
                    <div class="p-4" style="background: {{ old('secondary_color', $customization->secondary_color ?? '#f3f4f6') }}">
                        @if ($customization->logo)
                            <img src="{{ asset('storage/'.$customization->logo) }}" alt="Logo atual" class="mb-3 max-h-16 object-contain">
                        @endif
                        <div class="text-xl font-bold" style="color: {{ old('primary_color', $customization->primary_color ?? '#2563eb') }}">
            ...
            <section class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
                ...
                <button class="mt-5 w-full rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-700">
                    Salvar customização
                </button>
                @if (auth()->user()->tenant?->slug)
                    <a href="/{{ auth()->user()->tenant->slug }}" class="mt-3 block text-center text-sm font-semibold text-blue-600 hover:text-blue-700">
                        Ver microssite
                    </a>
                @endif
            </section>
